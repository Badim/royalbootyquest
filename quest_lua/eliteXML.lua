...
--
-- CHANGELOG:
-- 1.3 - added support of underscope.
-- 1.2 - added get_xml_by_cname, get_xml_by_attr.
...
function _G.get_xml_by_cname(xml,val)
	for i=1,#xml.child do
		if(xml.child[i].name == val)then
			return xml.child[i]
		end
	end
	return nil;
end
function _G.get_xml_by_attr(xml, attr, val)
	for i=1, #xml.child do
		if(xml.child[i].properties[attr] == val)then
			return xml.child[i]
		end
	end
	return nil;
end
function _G.get_list_by_attr(xml, attr, val)
	local list = {};
	for i=1, #xml.child do
		if(xml.child[i].properties[attr] == val)then
			table.insert(list, xml.child[i]);
		end
	end
	return list;
end
function _G.get_xml_contains_attr(xml, attr, val, del)
	for i=1, #xml.child do
		local str = xml.child[i].properties[attr];
		if(xml.child[i].properties[attr..'_arr'] == nil)then
			xml.child[i].properties[attr..'_arr'] = string_split(str, del);
		end
		local arr = xml.child[i].properties[attr..'_arr'];
		for j=1,#arr do
			if(arr[j] == val)then
				return xml.child[i]
			end
