...
return function(url)
local Firebase = {url = url, authToken = ''};
...
function Firebase:request(ref, method, data, query, callback)
	...
	network.request(url, method, function(event)
		print("request@url:", 'data: '..tostring(#event.response), url);
		callback(event);
	end, params);
end
...
function Firebase:on(ref, query, callback)
	...
	local streamed_get = function(event)
	    ...
	    local temp_path = system.pathForFile( "fb/", system.TemporaryDirectory )
